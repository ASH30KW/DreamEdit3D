...
class SpatialDreambooth:
    ...
    def main(self):
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                if self.args.phase1_train_steps == global_step:
                    ...
                    learned_embeds = self.text_encoder.get_input_embeddings().weight.data[self.placeholder_token_ids[0]].clone()
                    torch.save(learned_embeds, f"{self.args.output_dir}/learned_embedding.pt")
                    torch.save(self.placeholder_tokens[0], f"{self.args.output_dir}/placeholder_token.pt")                    
        ...
        if self.args.config_path is None:
            model = build_model(self.args.model_name, ckpt_path=self.args.ckpt_path)
        else:
            assert self.args.ckpt_path is not None, "ckpt_path must be specified!"
            config = OmegaConf.load(self.args.config_path)
            model = instantiate_from_config(config.model)
            model.load_state_dict(torch.load(self.args.ckpt_path, map_location='cpu'))

        # Try to set device attribute, but catch error if it's read-only
        try:
            model.device = self.accelerator.device
        except (AttributeError, TypeError):
            pass  # device is a read-only property in some PyTorch versions

        model.to(self.accelerator.device)
        model.eval()

        sampler = DDIMSampler(model)
        uc = model.get_learned_conditioning( [""] ).to(self.accelerator.device)
        ...
        # t = self.args.text + self.args.suffix
        
        # Load learned embedding and token
        learned_embed = torch.load(f"{self.args.output_dir}/learned_embedding.pt")
        placeholder_token = torch.load(f"{self.args.output_dir}/placeholder_token.pt")

        # Inject the learned embedding into MVDream's text encoder
        ...
        if hasattr(text_encoder, 'tokenizer'):
            ...
            if placeholder_token not in tokenizer.get_vocab():
                ...
                new_token_id = tokenizer.convert_tokens_to_ids(placeholder_token)
            else:
                new_token_id = tokenizer.convert_tokens_to_ids(placeholder_token)
        else:
            # OpenCLIP tokenizer (FrozenOpenCLIPEmbedder) - use a fixed approach
            # OpenCLIP has a fixed vocabulary, so we'll use an existing token ID for the placeholder
            import open_clip
            # Use a less common token as placeholder, e.g., the "!" token
            test_tokens = open_clip.tokenize([placeholder_token, "!", "<|endoftext|>"])
            if len(test_tokens[0].nonzero()) > 1:  # If tokenizer recognizes the placeholder
                new_token_id = test_tokens[0][test_tokens[0].nonzero()[0][0]].item()
            else:
                # Fallback to using a less common token
                fallback_tokens = open_clip.tokenize(["!"])
                new_token_id = fallback_tokens[0][fallback_tokens[0].nonzero()[0][0]].item()
            print(f"OpenCLIP: Using token ID {new_token_id} for placeholder '{placeholder_token}'")
        
        print(f"Placeholder token '{placeholder_token}' has ID: {new_token_id}")

        # Get the embedding layer - MVDream uses OpenCLIP structure
        try:
            ...
            if new_token_id >= vocab_size:
                ...
                new_embedding = torch.nn.Embedding(new_token_id + 1, embed_dim)
                new_embedding.weight.data[:vocab_size] = embedding_layer.weight.data
                
                # Initialize new token embeddings with learned embedding
                new_embedding.weight.data[new_token_id] = learned_embed.to(embedding_layer.weight.device)
                
                # Replace the embedding layer
                text_encoder.model.token_embedding = new_embedding
                print(f"Resized embedding layer to {new_token_id + 1} tokens and injected learned embedding")
            else:
                # Token already exists, just replace its embedding
                embedding_layer.weight.data[new_token_id] = learned_embed.to(embedding_layer.weight.device)
                print(f"Replaced existing token embedding at position {new_token_id}")
            
        except Exception as e:
            print(f"Could not inject embedding: {e}")
            print(f"Text encoder type: {type(text_encoder)}")
            if hasattr(text_encoder, 'model'):
                print(f"Model attributes: {dir(text_encoder.model)}")
            print("Using token without injection")

        # Use the placeholder token in your text
        t = self.args.text.replace("PLACEHOLDER", placeholder_token) + self.args.suffix
        print('ttt',t)
        set_seed(self.args.seed)
        images = []
        for j in range(3):
            img = t2i(model, self.args.size, t, uc, sampler, step=50, scale=10, batch_size=self.args.sample_batch_size, ddim_eta=0.0, 
                    dtype=self.weight_dtype, device=self.accelerator.device, camera=camera, num_frames=self.args.num_frames)
